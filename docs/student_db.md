# IBE Student-Alumni Database

## Overview
This repository contains the schema and documentation for IBE student alumni database built in **Supabase (PostgreSQL)**. The database stores student profiles, education history, career records, and involvement in organizations.
...
|-------------------|----------------|---------------------------------------|
| id                | UUID           | Primary key                          |
| user_id           | UUID           | Foreign key to `auth.users.id` (Supabase Auth) |
